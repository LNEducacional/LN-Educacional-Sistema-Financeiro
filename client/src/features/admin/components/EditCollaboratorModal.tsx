...
import { X, Pencil, User, Mail, Sparkles, Key, Star } from 'lucide-react';
import { useUpdateCollaborator } from '../api';
import type { CollaboratorDetail, UpdateCollaboratorRequest } from '../types';
...
export function EditCollaboratorModal({
  ...
  const [specialty, setSpecialty] = useState(collaborator.specialty || '');
  const [pixKey, setPixKey] = useState(collaborator.pix_key || '');
  const [internalRanking, setInternalRanking] = useState(
    collaborator.internal_ranking.toString()
  );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const data: UpdateCollaboratorRequest = {};

    if (specialty !== (collaborator.specialty || '')) {
      data.specialty = specialty || undefined;
    }
    if (pixKey !== (collaborator.pix_key || '')) {
      data.pix_key = pixKey || undefined;
    }
    const newRanking = parseFloat(internalRanking);
    if (!isNaN(newRanking) && newRanking !== collaborator.internal_ranking) {
      data.internal_ranking = newRanking;
    }
  ...
  const inputDisabledClass = "w-full rounded-2xl bg-black/20 text-gray-500 border border-white/5 px-10 py-2.5 text-sm cursor-not-allowed";
        ...
        <div className="p-6 sm:p-8">
          ...
          <div className="mb-6 flex items-start justify-between">
            <div>
              ...
              <p className="text-sm text-gray-400 mt-1">Atualize os dados do colaborador.</p>
            </div>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {/* Campos somente leitura - grid 2 colunas no desktop */}
            ...
            <div className="border-t border-white/10 pt-4">
              <p className="text-xs text-gray-500 mb-3 pl-2.5">Campos editaveis</p>
            </div>

            {/* Campos editaveis - grid 2 colunas no desktop */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {/* Especialidade */}
              <div>
                <label className="block text-xs font-medium text-gray-300 mb-1.5 pl-2.5">
            ...
            >
              ...
              <span className="absolute right-[0.3em] z-10 flex items-center justify-center h-[2.2em] w-[2.2em] rounded-[0.7em] bg-white/10 transition-all duration-300 group-hover:bg-white/20 group-hover:w-[calc(100%-0.6em)]">
                {updateMutation.isPending ? (
                  ...
                  <Pencil className="w-[1.1em] h-[1.1em] text-gray-300 transition-all duration-300 group-hover:text-white group-hover:scale-110" />
                )}
              </span>
            </button>
          </form>
